...
(defparameter *global-runs 0)
(defun simple-increment ()
  (declare (optimize (speed 3) (safety 0)))
  (incf (the fixnum *global-runs)))

(defun perf-method-call ()
  (declare (optimize (speed 3) (safety 0)))
...
(defun double-method-call ()
  (simple-increment))

(defun perf-double-method-call ()
  (declare (optimize (speed 3) (safety 0)))
  (let ((iterations 100000000))
    (do ((rep iterations (1- rep))
         (start (get-internal-real-time)))
...
(defclass my-test-object () ())
(defgeneric class-method-call (object))
(defmethod class-method-call ((o my-test-object))
  (declare (optimize (speed 3) (safety 0)))
  (incf (the fixnum *global-runs)))

(defun perf-class-method-call ()
  (declare (optimize (speed 3) (safety 0)))
  (let ((iterations 100000000)
        (object (make-instance 'my-test-object)))
    (do ((rep iterations (1- rep))
         (start (get-internal-real-time)))
        ((zerop rep) (round (/ (* 1000 iterations)
                               (- (get-internal-real-time) start))))
      (declare (type fixnum rep))
      (class-method-call object))))

(defun perf-parallel-rep ()
  (declare (optimize (speed 3) (safety 0)))
  (let ((iterations 10000000000))
    (let ((thr1 (bt:make-thread
      ...
      (do ((rep iterations (1- rep))
           (start (get-internal-real-time)))
          ((zerop rep) (progn (bt:join-thread thr1)
                              (bt:join-thread thr2)
                              (bt:join-thread thr3)
                              (* 4 (round (/ (* 1000 iterations)
                                             (- (get-internal-real-time) start))))))
        (declare (type fixnum rep))))))
